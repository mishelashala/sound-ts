...
var isObjectLiteralExpression = (...args) => ts.isObjectLiteralExpression(...args);
...
function rewriteMethodsAsProperties(source) {
  ...
  const collectOverloads = (node) => {
    if (isMethodDeclaration(node) && node.name) {
      ...
      if (isClassDeclaration(parent) || isClassExpression(parent) || isObjectLiteralExpression(parent)) {
  ...
  const visit = (node) => {
    ...
    if (isMethodDeclaration(node) && node.name) {
      const parent = node.parent;
      const inClassOrObject = isClassDeclaration(parent) || isClassExpression(parent) || isObjectLiteralExpression(parent);
      if (inClassOrObject) {
        const key = `${parent.pos}:${methodKey(node, sf)}`;
...
var PREFIX_NOT_KEYWORDS = /* @__PURE__ */ new Set([
  "return",
  "throw",
  "typeof",
  "void",
...
function skipWsBack2(scan, i) {
  while (i >= 0 && /[\s\n\r\t]/.test(scan[i])) i--;
  return i;
}
function readIdentBack(scan, endIncl) {
...
function isNonNullOrDefiniteAssignment(scan, i) {
  if (scan[i] !== "!") return false;
  if (scan[i + 1] === "=") return false;
  const prev = skipWsBack2(scan, i - 1);
...
function formatWhere(filename, source, index) {
  if (filename === void 0) return `offset ${index}`;
  let line = 1;
  let col = 1;
  for (let i = 0; i < index && i < source.length; i++) {
    if (source[i] === "\n") {
...
function formatWhere2(filename, offset) {
...
function assertNoStructuralAliases(source, filename) {
  ...
  while ((m = ifaceRe.exec(scan)) !== null) {
    const name = m[1];
    throw new SyntaxError(
      `structural interface '${name}' is not allowed in .sts (${formatWhere2(filename, m.index)}); use 'validate type' or 'brand type' instead`
    );
  }
  ...
  while ((m = typeRe.exec(scan)) !== null) {
    ...
    throw new SyntaxError(
      `structural object alias '${name}' is not allowed in .sts (${formatWhere2(filename, m.index)}); use 'validate type' or 'brand type' instead`
    );
  }
}

// packages/core/src/soundness/rejectAs.ts
var IDENT3 = /[A-Za-z_$][\w$]*/;
function skipWs5(source, i) {
  while (i < source.length && /[\s\n\r\t]/.test(source[i])) i++;
  return i;
}
...
function maskImportExportBindings(scan) {
...
function findAsAssertion(scan) {
  const re = /\bas\b/g;
  let m;
  while ((m = re.exec(scan)) !== null) {
    const start = m.index;
    let pos = skipWs5(scan, start + 2);
    if (/^const\b/.test(scan.slice(pos))) continue;
    const rest = scan.slice(pos);
    const startsType = new RegExp(`^${IDENT3.source}`).test(rest) || rest.startsWith("{") || rest.startsWith("(") || rest.startsWith("[") || rest.startsWith("'") || rest.startsWith('"') || rest.startsWith("`");
    if (!startsType) continue;
    return start;
  }
  return -1;
}
function findAngleBracketAssertion(scan, source) {
  for (let i = 0; i < scan.length; i++) {
    if (scan[i] !== "<") continue;
    let prev = i - 1;
    while (prev >= 0 && /[\s\n\r\t]/.test(scan[prev])) prev--;
    if (prev >= 0 && isWordChar(scan[prev])) continue;
    let pos = skipWs5(scan, i + 1);
    const name = scan.slice(pos).match(IDENT3);
    if (!name) continue;
    pos = skipWs5(scan, pos + name[0].length);
    if (scan[pos] !== ">") continue;
    const afterGt = pos + 1;
    const exprPos = skipWs5(source, afterGt);
    const next = source[exprPos];
    if (next === void 0) continue;
    if (isWordChar(next) || next === '"' || next === "'" || next === "`" || next === "(" || next === "[" || next === "{" || next === "+" || next === "-" || next === "!" || next === "~" || /[0-9]/.test(next)) {
      return i;
    }
  }
  return -1;
...
function locPrefix2(filename, source, index) {
  if (filename === void 0) return "";
  let line = 1;
  let col = 1;
  for (let i = 0; i < index && i < source.length; i++) {
...
function skipWsBack3(scan, i) {
  while (i >= 0 && /[\s\n\r\t]/.test(scan[i])) i--;
  return i;
}
function endsWithWord(scan, end, word) {
...
function isEqualsTypeContext(scan, eqIdx) {
  let i = skipWsBack3(scan, eqIdx - 1);
  if (i < 0) return false;
  if (!/[A-Za-z0-9_$]/.test(scan[i])) return false;
  while (i >= 0 && /[A-Za-z0-9_$]/.test(scan[i])) i--;
  i = skipWsBack3(scan, i);
...
function isTypePosition(scan, index) {
  let i = skipWsBack3(scan, index - 1);
  if (i < 0) return false;
  const c = scan[i];
  if (c === ":" || c === "|" || c === "&" || c === "<" || c === ",") {
    return true;
...
function throwWide(kind, filename, source, offset) {
  const where = locPrefix2(filename, source, offset);
  throw new SyntaxError(
    `${where}type '${kind}' is too wide for .sts (same hole as a weaker any). Use a specific type, lowercase object, a real object shape, or a function type literal \u2014 not rewritten to unknown.`
  );
}
